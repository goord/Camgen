...
#ifndef CAMGEN_EUCL_MAJ_BASIS_H_
#define CAMGEN_EUCL_MAJ_BASIS_H_

#include <Camgen/Euclidean.h>
#include <Camgen/Dirac_alg.h>

/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 * Euclidean Majorana basis type definition. It provides a matrix representation *
 * of the Clifford algebra in Euclidean spacetimes. It can be derived from the   *
 * Majorana basis in Minkowski space by multiplying all spacelike matrices with  *
 * the imaginary unit.                                                           *
 *                                                                               *
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
...
namespace Camgen
{
    class Euclidean_Majorana_basis
    {
	public:
    ...
    template<class value_t>class Euclidean_Majorana_basis::implementation<value_t,4,true>: public Dirac_algebra<value_t,implementation<value_t,4,true>,4,true>
    {
	public:
		...
		base_type::g[1][0][0]=value_type(-1,0);
		base_type::g[1][1][1]=value_type(1,0);
		base_type::g[1][2][2]=value_type(-1,0);
		base_type::g[1][3][3]=value_type(1,0);
		...
		base_type::g[3][0][1]=value_type(1,0);
		base_type::g[3][1][0]=value_type(1,0);
		base_type::g[3][2][3]=value_type(1,0);
		base_type::g[3][3][2]=value_type(1,0);
		...
		base_type::C_matrix[0][2]=value_type(-1,0);
		base_type::C_matrix[1][3]=value_type(-1,0);
		base_type::C_matrix[2][0]=value_type(-1,0);
		base_type::C_matrix[3][1]=value_type(-1,0);

	    }
    };

    template<class value_t>const int Euclidean_Majorana_basis::implementation<value_t,4,true>::eta_g;
    template<class value_t>const int Euclidean_Majorana_basis::implementation<value_t,4,true>::eta_g5;
    template<class value_t>const int Euclidean_Majorana_basis::implementation<value_t,4,true>::eta_gg5;
    template<class value_t>const int Euclidean_Majorana_basis::implementation<value_t,4,true>::eta_gg;
}
#endif /*CAMGEN_EUCL_MAJ_BASIS_H_*/
