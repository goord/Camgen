...
#ifndef CAMGEN_VFFR_H_
#define CAMGEN_VFFR_H_

#include <Camgen/def_args.h>

/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 * Right-handed vector-fermion-fermion Feynman rule declaration and definition.*
 *                                                                             *
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
...
namespace Camgen
{
    ...
    template<class model_t>class vffR
    {
	public:
    ...
    template<class model_t>const std::size_t vffR<model_t>::vector_size;
    template<class model_t>const std::size_t vffR<model_t>::spinor_size;
    template<class model_t>const std::size_t vffR<model_t>::rank;
    template<class model_t>const std::size_t vffR<model_t>::params;
    template<class model_t>const std::size_t vffR<model_t>::tensor_size;
    template<class model_t>const bool vffR<model_t>::p_dependent;
    template<class model_t>const bool vffR<model_t>::fermionic;
    template<class model_t>const std::size_t vffR<model_t>::sizes[4]={vffR<model_t>::vector_size,vffR<model_t>::spinor_size,vffR<model_t>::spinor_size,0};
    template<class model_t>const std::string vffR<model_t>::formula="g_(mu1)(1-g_5)(i2,j3)/2";

    /* Specialisation of the evaluate class template: */

    template<class model_t>class evaluate< vffR<model_t> >
    {
	public:

	    /* Reference type definition of the vertex type: */

	    typedef vffR<model_t> vertex_type;
	...
	public:
	    ...
	    /* Recursive relations, calling the ggR-recursive relations in the
	     * Dirac algebra class: */

	    static void first(ARG_LIST)
	    {
		Dirac_algebra_type::ggR_first(factor*C_0,A_0,A_1,A_2);
	    }
	    static void second(ARG_LIST)
	    {
		Dirac_algebra_type::ggR_second(factor*C_0,A_0,A_1,A_2);
	    }
	    static void third(ARG_LIST)
	    {
		Dirac_algebra_type::ggR_third(factor*C_0,A_0,A_1,A_2);
	    }
	    static void fourth(ARG_LIST){}

	    /* Right charge-conjugate recursive relations, calling the ggR_C-recursive
	     * relations in the Dirac algebra class: */

	    static void first_C(ARG_LIST)
	    {
		Dirac_algebra_type::ggR_C_first(factor*C_0,A_0,A_1,A_2);
	    }
	    static void second_C(ARG_LIST)
	    {
		Dirac_algebra_type::ggR_C_second(factor*C_0,A_0,A_1,A_2);
	    }
	    static void third_C(ARG_LIST)
	    {
		Dirac_algebra_type::ggR_C_third(factor*C_0,A_0,A_1,A_2);
	    }
	    static void fourth_C(ARG_LIST){}

	    /* Left charge-conjugate recursive relations, calling the Cc_ggR-recursive
	     * relations in the Dirac algebra class: */

	    static void Cc_first(ARG_LIST)
	    {
		Dirac_algebra_type::Cc_ggR_first(factor*C_0,A_0,A_1,A_2);
	    }
	    static void Cc_second(ARG_LIST)
	    {
		Dirac_algebra_type::Cc_ggR_second(factor*C_0,A_0,A_1,A_2);
	    }
	    static void Cc_third(ARG_LIST)
	    {
		Dirac_algebra_type::Cc_ggR_third(factor*C_0,A_0,A_1,A_2);
	    }
	     ...
	     * ggL-relations, multiplied with the V-sign. */
	     ...
	     * opposite, apply the ggL_C-relations, multiplied with the V-sign.
	     * */
	     ...
	     * and A-signs are equal, multiply by -1 if they are negative, if
	     * opposite, apply the Cc_ggL-relations, multiplied with the V-sign.
	     * */
    ...
    template<class model_t>const std::size_t evaluate< vffR<model_t> >::vector_size;
    template<class model_t>const std::size_t evaluate< vffR<model_t> >::spinor_size;
    template<class model_t>const int evaluate< vffR<model_t> >::Vsign;
    template<class model_t>const int evaluate< vffR<model_t> >::Asign;
    template<class model_t>const int evaluate< vffR<model_t> >::Psign;
}

#include <Camgen/undef_args.h>

#endif /*CAMGEN_VFFR_H_*/
