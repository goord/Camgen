...
#ifndef CAMGEN_EUCL_WEYL_BASIS_H_
#define CAMGEN_EUCL_WEYL_BASIS_H_

#include <Camgen/Euclidean.h>
#include <Camgen/Dirac_alg.h>

/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 * Euclidean Weyl basis type definition. It provides a matrix representation of  *
 * the Clifford algebra in Euclidean spacetimes. It can be derived from the      *
 * Weyl basis in Minkowski space by multiplying all spacelike matrices with the  *
 * imaginary unit.                                                               *
 *                                                                               *
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
...
namespace Camgen
{
    ...
    template<class value_t>class Euclidean_Weyl_basis::implementation<value_t,4,true>: public Dirac_algebra<value_t,implementation<value_t,4,true>,4,true>
    {
	public:
		...
		base_type::C_matrix[0][1]=value_type(0,1);
		base_type::C_matrix[1][0]=value_type(0,-1);
		base_type::C_matrix[2][3]=value_type(0,-1);
		base_type::C_matrix[3][2]=value_type(0,1);
		
	    }
    };
    template<class value_t>const int Euclidean_Weyl_basis::implementation<value_t,4,true>::eta_g;
    template<class value_t>const int Euclidean_Weyl_basis::implementation<value_t,4,true>::eta_g5;
    template<class value_t>const int Euclidean_Weyl_basis::implementation<value_t,4,true>::eta_gg5;
    template<class value_t>const int Euclidean_Weyl_basis::implementation<value_t,4,true>::eta_gg;
}

#endif /*CAMGEN_EUCL_WEYL_BASIS_H_*/
